...
項目は、 **Items** クラスの [**CreateItemAsync**](https://aka.ms/CosmosDotnetAPIDocs) 関数を使って作成することができます。SQL API を使うとき、項目は、ユーザー定義の（任意の） JSON コンテンツであるドキュメントに変換されます。Azure Cosmos DB コンテナーに項目を挿入してみましょう。
